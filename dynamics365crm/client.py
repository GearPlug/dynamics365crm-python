    ...
    def set_auth_token_via_tenant(self, tenant_id, scope):
        ...
        if tenant_id and scope and self.client_id and self.client_secret:
            ...
            response = self.parse_response(
                requests.post(url, headers=headers, data=args)
            )
            self.set_token(response["access_token"])
            return
        raise Exception(
            "The attributes necessary to obtain the token were not obtained/invalid - {tenant_id, scope, client_id, client_secret}"
        )
